...
- For `/agentfactory`, treat Hermes agents as governed enterprise operating units, not generic prompts. Require taste alignment, 12-question intent intake, deep research, an Agent Factory workflow artifact, `HERMES-{NAME}-SPEC.md`, manifest, least-privilege capability stack, memory-coherent seed, independent verification, registry update, and tested kill switch before production readiness.
