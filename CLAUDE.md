...
18. **Definitive Workflow Command**: `/opusworkflow` is the command developers should use for normal mutating work. It means Opus 4.7 high planner/reviewer when proven available plus MiniMax-M2.7-highspeed Token Plan executor for bounded bulk work, and it must close as verified, partial, or blocked-with-repair.
...
20. **OpusWorkflow Default**: `/opusworkflow` is the definitive effectiveness-first, cost-aware route over `/opusminimax --mode workflow` and the default for all mutating work: Opus 4.7 high only at judgment gates when proven available, MiniMax-M2.7-highspeed Token Plan for bulk execution, executor concurrency 1 by default for Plus-Highspeed, and closeout only as verified, partial, or blocked-with-repair. Default effort is `high`; `xhigh` and `max` are explicit overrides. The default `--plan-mode-policy auto` records `auto_approved_when_gates_pass` and starts execution automatically only after research, code audit, `/introspect pre-plan`, Agent-Native Estimate, `SPEC.md`, and `/specqa` allow execution. Mutating specialist routes keep their own contracts as `inner_contract` values under this outer route.
21. **SonnetMiniMax Shortcut**: `/sonnetminimax` is the power-user Opus-saving sibling route. It keeps the same `/opusworkflow` governance but requests `claude-sonnet-4-6` for planning/review/judgment and `MiniMax-M2.7-highspeed` from the MiniMax Token Plan for bounded execution, defaulting to `--effort max`. Use it when Opus is scarce or exhausted; do not present it as the main route.
22. **Model Profile Freedom**: `/opusworkflow --model-profile minimax|sonnetminimax|opussonnet|sonnet|opus|default|custom` lets the operator choose governed model routing without changing the default. `sonnetminimax` requests Sonnet 4.6 judgment with MiniMax execution; Anthropic-only profiles must stay provider-neutral and never claim runtime identity without proof.
23. **OpusSonnet Option**: `/opussonnet` is an optional Claude-only suggested route for installs created with `setup.sh --mode opussonnet`. It requests Claude Code `opusplan`, pins Opus 4.7 for planning/judgment and Sonnet 4.6 for execution, and requires no MiniMax token. Do not present it as the default MiniMax-backed budget strategy or claim runtime model identity without proof.
24. **OpusSolo Option**: `/opusolo` is an optional all-Opus sibling route for installs created with `setup.sh --mode opusolo`. It pins Opus 4.7 for planning, execution, Spec QA, adversarial review, and final judgment; default effort is `high`, and `--effort max` maps to the highest available Claude CLI effort alias. Do not present it as the default cost-aware strategy or claim runtime model identity without proof.
...
2. Use `/opusworkflow` as the outer route by default: Opus 4.7 high handles judgment gates when identity is proven, and MiniMax-M2.7-highspeed Token Plan handles bounded bulk execution
...
**When you say `/opusworkflow` or give a normal build/plan task:** run `/opusminimax` in workflow mode with stricter cost policy: request Opus 4.7 high by default only for plan/spec freeze, adversarial review, and final judgment when identity is proven; use MiniMax-M2.7-highspeed Token Plan for coding packets and repair loops; keep Plus-Highspeed executor concurrency at 1 unless provider evidence proves more; record `plan_mode.policy=auto` and `auto_approved_when_gates_pass` only after the research, audit, pre-plan introspection, estimate, spec, and Spec QA gates pass; close only as verified, partial, or blocked-with-repair.

**When you say `/sonnetminimax`:** run the same governed lifecycle as `/opusworkflow`, but use the Opus-saving contract: `claude-sonnet-4-6` for planning, Spec QA/review, adversarial judgment, and final decision work; `MiniMax-M2.7-highspeed` from the MiniMax Token Plan for bounded execution; default effort `max`; runtime identity claims blocked until `/status`, sentinel, or durable artifact proof exists.

**When you specify a model profile:** honor `/opusworkflow --model-profile sonnetminimax|sonnet|opus|opussonnet|default|custom` as an explicit operator choice while keeping the same SPEC, introspection, verification, no-secret, and runtime-identity-proof rules. `sonnetminimax` is MiniMax-backed Sonnet planning/review; `sonnet` is all-Sonnet Anthropic-only; `/opussonnet` is Claude-only Opus planning plus Sonnet execution.
...
**When you say `/opusolo`:** run the same governed lifecycle as `/opusworkflow`, but use the optional all-Opus contract: `claude-opus-4-7` for planning, execution, Spec QA, adversarial review, and final judgment. Default effort is `high`; honor `--effort max` as the explicit highest-effort alias. Do not require MiniMax, do not silently downgrade to Sonnet/default/MiniMax, and do not claim runtime model proof without `/status`, a sentinel, or artifact evidence.
...
| /opusworkflow | Definitive workflow command for mutating work: Opus 4.7 high planner/reviewer when proven available plus MiniMax-M2.7-highspeed Token Plan execution through `/opusminimax --mode workflow`; supports `sonnetminimax` for Sonnet 4.6 judgment plus MiniMax execution; auto-approves the plan-to-execution transition only when gates pass; closes as verified, partial, or blocked-with-repair |
...
| /sonnetminimax | Power-user Opus-saving route: Sonnet 4.6 planning/review/judgment plus MiniMax-M2.7-highspeed Token Plan execution, default effort max, same `/opusworkflow` governance |
...
| /opusolo | Optional all-Opus route: Opus 4.7 for planning, execution, Spec QA, review, and final judgment, default effort high, optional `--effort max`, no MiniMax token required |
...
- **Runtime Effectiveness Hooks**: `.claude/settings.json` wires `.claude/hooks/govern-effectiveness.sh` into Claude Code `PreToolUse`, `Stop`, and `SubagentStop` events. A Stop hook block is repair feedback: positive closeout needs commands or verification evidence; read-only closeout may cite files inspected or sources reviewed; "tests not run" or "unverified" must close as partial/blocked, not done. The overlapping closeout hooks use the deterministic `agentcloseout-physics` engine under `tools/agentcloseout-physics/`; ACSP-CC is still only a proposed closeout profile, and conformance output is self-assessed preflight evidence, not certification or standard adoption. Do not claim hook enforcement unless `bash scripts/hook-smoke.sh` and `bash scripts/agentcloseout-physics-smoke.sh` pass.
...
- **OpusWorkflow Budget**: `/opusworkflow` is the mutating-work default and primary best-results route for the Claude + MiniMax Token Plan strategy. It requests Opus 4.7 high for judgment by default, must not run Opus as a bulk executor, must not silently use PAYG, and must record `outer_route`, `inner_contract`, `outcome_policy=verified-partial-or-blocked-with-repair`, `planner_identity_status`, `executor_identity_status`, `fallback_status`, `plan_mode.policy=auto`, `auto_approved_when_gates_pass`, and `provider_ceiling=1` until runtime MiniMax tier evidence proves a higher safe executor budget.
- **SonnetMiniMax Shortcut**: `/sonnetminimax` is the short form for `--model-profile sonnetminimax --effort max`. It exists for power users preserving Opus quota, and must keep MiniMax as `MiniMax-M2.7-highspeed` from the Token Plan with conservative concurrency until runtime evidence proves otherwise.
- **Governed Model Profiles**: Explicit `--model-profile sonnetminimax|sonnet|opus|opussonnet|default|custom` overrides are allowed, but the artifact must record `model_profile`, `model_route`, requested planner/executor models, provider boundaries, and identity status. `sonnetminimax` requests `claude-sonnet-4-6` for planner/reviewer and `MiniMax-M2.7-highspeed` for execution; static profile selection is not runtime proof.
...
- **OpusSolo Suggested Profile**: `.claude/settings.opusolo.example.json` is the optional all-Opus profile. It pins `claude-opus-4-7`, defaults effort to `high`, supports `/opusolo --effort max` through the wrapper, and must not contain MiniMax base URLs or credentials.
