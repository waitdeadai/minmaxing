...
| /opusworkflow | Definitive workflow command for mutating work: Opus 4.7 high/xhigh planner/reviewer when proven available plus MiniMax-M2.7-highspeed execution through `/opusminimax --mode workflow`; supports `sonnetminimax` for Sonnet 4.6 judgment plus MiniMax execution; auto-approves the plan-to-execution transition only when gates pass; closes as verified, partial, or blocked-with-repair |
