...
| goal mode, native /goal, /goal <condition>, keep working until, bounded continuation | route to `/goal-mode` for static readiness, Goal Assist copy-paste templates, and no-shadow diagnostics; use native `/goal <condition>` manually only around already-known failed gates with owned scope, forbidden paths, transcript evidence, stop bounds, and parent verification, never as `/workflow`, `/parallel`, or `/verify` replacement |
